...
class FAControlAgent(BaseAgent):
    ...
    def choose_action_egreedy(self, q_s):
        ...
        if self.rand_generator.rand() < self.epsilon:
            ...
            action = argmax(self.rand_generator, q_s)

        return action

    def choose_action_greedy(self, q_s):
    ...
    def set_q_params(self, q_params):
        """sets the agent's q function parameters to the given vector.
...
class MLPControlAgent():

    """
    Implements an MLP action-value function approximator,
    input is state s vector
    output is vector where ith component is value of action (s, i)
    """
