...
class RVIQlearningAgent(MLPControlAgent):
    """
    Implements the R-learning algorithm by Schwartz (1993).
    """
    ...
    def agent_start(self, observation):
        action = super().agent_start(observation)
        self.references_actions.append(action)
        self.references_states.append(observation)
        return action
    ...
    def agent_step(self, reward, observation):
        ...
        if self.frame_count < self.num_reference_states:
            self.references_actions.append(action)
            self.references_states.append(observation)
        ...
        #'''
        if self.frame_count % self.update_avg_reward == 0 and self.use_avg_reward == True and self.frame_count > self.num_reference_states:
            ### average reward batch update
            # sample previous 'update_target_network' samples from replay buffer
            state_sample = np.array(self.references_states)
            action_sample = np.array(self.references_actions)

            masks = tf.one_hot(action_sample, self.num_actions, dtype=tf.float64)
            
            q_values = self.get_batch_value(state_sample)
            q_action_mean = tf.reduce_mean(tf.reduce_sum(tf.multiply(q_values, masks), axis=1))
            
            # update average reward
            self.avg_reward = q_action_mean
        #'''
